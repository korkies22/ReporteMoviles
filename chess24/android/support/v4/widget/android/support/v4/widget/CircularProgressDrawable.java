...
implements Animatable {
    ...
    private void applyTransformation(float f, Ring ring, boolean bl) {
        ...
        if (f != 1.0f || bl) {
            ...
            if (f < 0.5f) {
                ...
                f3 = MATERIAL_INTERPOLATOR.getInterpolation(f4);
                f4 = f2;
                f2 = 0.79f * f3 + 0.01f + f2;
            } else {
                f4 = (f - 0.5f) / 0.5f;
                f2 = ring.getStartingStartTrim() + 0.79f;
                f4 = f2 - (0.79f * (1.0f - MATERIAL_INTERPOLATOR.getInterpolation(f4)) + 0.01f);
            }
            f3 = this.mRotationCount;
            ring.setStartTrim(f4);
            ring.setEndTrim(f2);
            ring.setRotation(f5 + 0.20999998f * f);
            this.setRotation(216.0f * (f + f3));
        }
    }
    ...
    private int evaluateColorChange(float f, int n, int n2) {
        ...
        int n4 = n >> 16 & 255;
        int n5 = n >> 8 & 255;
        return n3 + (int)((float)((n2 >> 24 & 255) - n3) * f) << 24 | n4 + (int)((float)((n2 >> 16 & 255) - n4) * f) << 16 | n5 + (int)((float)((n2 >> 8 & 255) - n5) * f) << 8 | n + (int)(f * (float)((n2 & 255) - (n &= 255)));
    }
