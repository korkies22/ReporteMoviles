...
public final class ColorUtils {
    ...
    public static int HSLToColor(@NonNull float[] arrf) {
        ...
        switch ((int)f / 60) {
            ...
            case 6: {
                ...
                n2 = Math.round(255.0f * f3);
                n = Math.round(255.0f * (f4 + f3));
                break;
            }
            case 4: {
                n3 = Math.round((f4 + f3) * 255.0f);
                n2 = Math.round(255.0f * f3);
                n = Math.round(255.0f * (f2 + f3));
                break;
            }
            case 3: {
                n3 = Math.round(255.0f * f3);
        ...
        return Color.rgb((int)ColorUtils.constrain(n3, 0, 255), (int)ColorUtils.constrain(n2, 0, 255), (int)ColorUtils.constrain(n, 0, 255));
    }
    ...
    public static void RGBToHSL(@IntRange(from=0L, to=255L) int n, @IntRange(from=0L, to=255L) int n2, @IntRange(from=0L, to=255L) int n3, @NonNull float[] arrf) {
        ...
        arrf[0] = ColorUtils.constrain(f, 0.0f, 360.0f);
        arrf[1] = ColorUtils.constrain(f6, 0.0f, 1.0f);
    ...
    public static void RGBToXYZ(@IntRange(from=0L, to=255L) int n, @IntRange(from=0L, to=255L) int n2, @IntRange(from=0L, to=255L) int n3, @NonNull double[] arrd) {
        ...
        arrd[2] = 100.0 * (d * 0.0193 + d2 * 0.1192 + d3 * 0.9505);
    }
    ...
    public static int XYZToColor(@FloatRange(from=0.0, to=95.047) double d, @FloatRange(from=0.0, to=100.0) double d2, @FloatRange(from=0.0, to=108.883) double d3) {
        ...
        d3 = (d * 0.0557 + d2 * -0.204 + 1.057 * d3) / 100.0;
        d = d4 > 0.0031308 ? Math.pow(d4, 0.4166666666666667) * 1.055 - 0.055 : 12.92 * d4;
        d2 = d5 > 0.0031308 ? Math.pow(d5, 0.4166666666666667) * 1.055 - 0.055 : 12.92 * d5;
        d3 = d3 > 0.0031308 ? 1.055 * Math.pow(d3, 0.4166666666666667) - 0.055 : 12.92 * d3;
