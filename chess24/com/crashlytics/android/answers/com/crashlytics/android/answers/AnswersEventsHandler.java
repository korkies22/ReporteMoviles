...
implements EventsStorageListener {
    ...
    public void setAnalyticsSettingsData(final AnalyticsSettingsData analyticsSettingsData, final String string) {
        this.executeAsync(new Runnable(){

            @Override
            public void run() {
                try {
                    AnswersEventsHandler.this.strategy.setAnalyticsSettingsData(analyticsSettingsData, string);
                    return;
                }
                catch (Exception exception) {
