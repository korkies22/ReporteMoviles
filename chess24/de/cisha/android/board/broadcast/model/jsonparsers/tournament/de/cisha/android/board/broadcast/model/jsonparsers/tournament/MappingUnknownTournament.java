...
public class MappingUnknownTournament
extends AbstractBroadcastTournamentModelMapping {
    ...
    public Tournament mapFromModel(TournamentModel tournamentModel) {
        Tournament tournament = new Tournament(tournamentModel.getTitle(), tournamentModel.getDescription(), BroadcastTournamentType.UNKNOWN);
        for (RoundModel roundModel : tournamentModel.getRounds()) {
            ...
            tournament.setPotentiallyNumbersOfGamesPerMatchForMainround(simpleTournamentRoundInfo, (List)tournamentModel.getMatchGroups().get(roundModel.getRoundNumber()));
            for (MatchModel matchModel : roundModel.getMatches()) {
                for (GameModel gameModel : matchModel.getGames()) {
                    TournamentGameInfo tournamentGameInfo = MappingUnknownTournament.createGameInfoFromModel(tournamentModel, roundModel, matchModel, gameModel);
                    ...
                    tournament.addGame(new Tournament.TournamentStructureKey((String)charSequence, (String)charSequence2, stringBuilder.toString()), simpleTournamentRoundInfo, tournamentGameInfo);
                }
            }
        }
        return tournament;
